...
const films_service_js_1 = require("../services/films.service.js");
...
const searchQuerySchema = zod_1.z.object({
    q: zod_1.z.string().max(100).trim().optional(),
    genre: zod_1.z.string().max(80).trim().optional(),
    decennie: zod_1.z.coerce.number().int().min(1900).max(2030).optional(),
    sort: zod_1.z.enum(["titre", "annee_desc", "annee_asc", "seances"]).optional(),
    page: zod_1.z.coerce.number().int().min(1).optional(),
    limit: zod_1.z.coerce.number().int().min(1).max(200).optional(),
    offset: zod_1.z.coerce.number().int().min(0).optional(),
});
const seancesQuerySchema = zod_1.z.object({
    ville: zod_1.z.string().trim().optional(),
    date: zod_1.z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Format attendu : YYYY-MM-DD").optional(),
    version: zod_1.z.nativeEnum(client_1.Version).optional(),
});
// ── Plugin Fastify ────────────────────────────────────────
const filmsRoutes = async (fastify) => {
    ...
    fastify.get("/films/all-classics", async (_request, reply) => {
        ...
        const films = await films_service_js_1.filmsService.getAllClassicFilms();
        await (0, redis_js_1.cacheSet)(cacheKey, films, 60 * 60 * 2); // 2h
        reply.header("X-Cache", "MISS");
        return films;
    });
    // ── GET /api/films?q= ─────────────────────────────────
    ...
    fastify.get("/films", async (request, reply) => {
        ...
        if (genre !== undefined || decennie !== undefined || sort !== undefined || page !== undefined) {
            ...
            const result = await films_service_js_1.filmsService.getCatalogFilms({
                q,
                genre,
                decennie,
                sort: sort,
            ...
            await (0, redis_js_1.cacheSet)(cacheKey, result, TTL);
            reply.header("X-Cache", "MISS");
            return result;
        }
        ...
        let films = await films_service_js_1.filmsService.searchFilms(q);
        if (offset)
            films = films.slice(offset);
        if (limit)
            films = films.slice(0, limit);
        await (0, redis_js_1.cacheSet)(cacheKey, films, TTL);
        reply.header("X-Cache", "MISS");
        return films;
    });
    // ── GET /api/films/:id ────────────────────────────────
    ...
    fastify.get("/films/:id", async (request, reply) => {
        ...
        await (0, redis_js_1.cacheSet)(cacheKey, film, TTL);
        reply.header("X-Cache", "MISS");
        return film;
    });
     ...
     */
    fastify.get("/films/:id/seances", async (request, reply) => {
        const { id } = request.params;
        ...
        const seances = await films_service_js_1.filmsService.getFilmSeances(id, filters);
        await (0, redis_js_1.cacheSet)(cacheKey, seances, TTL);
        reply.header("X-Cache", "MISS");
        return seances;
    });
     ...
     */
    fastify.get("/films/:id/dates", async (request, reply) => {
        const { id } = request.params;
        ...
        await (0, redis_js_1.cacheSet)(cacheKey, result, 60 * 15); // 15 min
        reply.header("X-Cache", "MISS");
        return result;
    });
     ...
     */
    fastify.get("/films/:id/trailer", async (request, reply) => {
        const { id } = request.params;
        ...
        if (TMDB_KEY && film.tmdbId) {
            try {
                ...
                const fetcher = (lang) => fetch(`${TMDB_BASE}/movie/${tmdbId}/videos?api_key=${TMDB_KEY}&language=${lang}`, {
                ...
                const isOfficialChannel = (v) => v.official ||
            ...
            const extractVideoIds = (html) => {
                ...
                for (const m of html.matchAll(/"videoId"\s*:\s*"([A-Za-z0-9_-]{11})"/g)) {
                    if (!ids.includes(m[1]))
                        ids.push(m[1]);
                    if (ids.length >= 5)
                        break; // garder les 5 premiers résultats
                }
                return ids;
            };
            const extractChannelIds = (html) => {
                ...
                for (const chunk of chunks.slice(1)) {
                    ...
                    if (vid.length === 11 && chMatch)
                        map.set(vid, chMatch[1]);
                }
                return map;
            };
        ...
        await (0, redis_js_1.cacheSet)(cacheKey, result, youtubeId ? 60 * 60 * 24 : 60 * 20);
        reply.header("X-Cache", "MISS");
        return result;
    });
     ...
     */
    fastify.get("/films/:id/rating", async (request, reply) => {
        const { id } = request.params;
        ...
        const agg = await prisma_js_1.prisma.avis.aggregate({
