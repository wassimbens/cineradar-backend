...
// ── Plugin ────────────────────────────────────────────────
const profilRoutes = async (fastify) => {
    // ── GET /api/profil/:email ────────────────────────────
    fastify.get("/profil/:email", async (req, reply) => {
        ...
        return reply.send({
            ...
            genresPreferes: user.genresPreferes ?? [],
    ...
    fastify.put("/profil/:email", async (req, reply) => {
        const { email } = req.params;
        ...
        if (genresPreferes !== undefined)
            updateData["genresPreferes"] = genresPreferes;
        // pseudo via profil update (legacy mode, sans JWT)
        if (req.body.pseudo !== undefined) {
            const newPseudo = req.body.pseudo ?? null;
            if (newPseudo) {
    ...
    fastify.post("/profil/:email/films-vus", async (req, reply) => {
        const { email } = req.params;
        const { filmId, cinemaId, dateVu } = req.body ?? {};
        if (!filmId)
            return reply.status(400).send({ error: "filmId requis" });
    ...
    fastify.delete("/profil/:email/films-vus/:id", async (req, reply) => {
    ...
    fastify.post("/profil/:email/favoris", async (req, reply) => {
        const { email } = req.params;
    ...
    fastify.put("/profil/:email/favoris/reorder", async (req, reply) => {
        const { email } = req.params;
        const items = req.body ?? [];
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user)
            return reply.send({ ok: true });
        await Promise.all(items.map(({ filmId, position }) => prisma.filmFavori.updateMany({
    ...
    fastify.delete("/profil/:email/favoris/:filmId", async (req, reply) => {
    ...
    fastify.post("/profil/:email/watchlist", async (req, reply) => {
        const { email } = req.params;
    ...
    fastify.delete("/profil/:email/watchlist/:filmId", async (req, reply) => {
        const { email, filmId } = req.params;
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user)
            return reply.send({ ok: true });
    ...
    fastify.post("/profil/:email/cinemas", async (req, reply) => {
        const { email } = req.params;
        const { cinemaId } = req.body ?? {};
        if (!cinemaId)
            return reply.status(400).send({ error: "cinemaId requis" });
        const user = await getOrCreateUser(email);
    ...
    fastify.delete("/profil/:email/cinemas/:cinemaId", async (req, reply) => {
    ...
    fastify.put("/profil/:email/avis/:filmId", async (req, reply) => {
        const { email, filmId } = req.params;
    ...
    fastify.delete("/profil/:email/avis/:filmId", async (req, reply) => {
        const { email, filmId } = req.params;
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user)
            return reply.send({ ok: true });
    ...
    fastify.get("/profils/:pseudo", async (req, reply) => {
        ...
        if (!user.isPublic)
            return reply.status(403).send({ error: "Ce profil est privé" });
        return reply.send({
            id: user.id,
            pseudo: user.pseudo,
            nom: user.nom,
    ...
    fastify.post("/profils/:pseudo/follow", async (req, reply) => {
        const authUser = (0, auth_js_1.extractUser)(req);
        if (!authUser)
    ...
    fastify.delete("/profils/:pseudo/follow", async (req, reply) => {
        const authUser = (0, auth_js_1.extractUser)(req);
        if (!authUser)
            return reply.status(401).send({ error: "Authentification requise" });
        const target = await prisma.user.findUnique({ where: { pseudo: req.params.pseudo } });
    ...
    fastify.get("/profils/:pseudo/followers", async (req, reply) => {
    ...
    fastify.get("/profils/:pseudo/following", async (req, reply) => {
        const user = await prisma.user.findUnique({ where: { pseudo: req.params.pseudo } });
        if (!user)
            return reply.status(404).send({ error: "Utilisateur introuvable" });
        const follows = await prisma.follow.findMany({
    ...
    fastify.get("/profils/:pseudo/is-following", async (req, reply) => {
        const authUser = (0, auth_js_1.extractUser)(req);
        if (!authUser)
    ...
    fastify.get("/profils/:pseudo/poster-choices", async (req, reply) => {
        const user = await prisma.user.findUnique({ where: { pseudo: req.params.pseudo } });
        if (!user)
            return reply.send({});
        ...
        for (const c of choices)
            map[c.filmId] = c.posterUrl;
        return reply.send(map);
    });
    // ── PUT /api/profil/:email/poster-choice ─────────────
    // Sauvegarde le choix d'affiche personnalisée d'un membre Pro
    fastify.put("/profil/:email/poster-choice", async (req, reply) => {
        const { email } = req.params;
        const { filmId, posterUrl } = req.body ?? {};
        if (!filmId || !posterUrl)
            return reply.status(400).send({ error: "filmId et posterUrl requis" });
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user)
            return reply.status(404).send({ error: "Utilisateur introuvable" });
        if (!user.isPremium)
            return reply.status(403).send({ error: "Fonctionnalité réservée aux membres Pro" });
    ...
    fastify.get("/profil/:email/poster-choices", async (req, reply) => {
        const { email } = req.params;
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user)
            return reply.send({});
    ...
    fastify.delete("/profil/:email/poster-choice/:filmId", async (req, reply) => {
        const { email, filmId } = req.params;
        const user = await prisma.user.findUnique({ where: { email } });
        if (!user)
            return reply.send({ ok: true });
    ...
    fastify.get("/profil/:email/recommandations", async (req, reply) => {
        const { email } = req.params;
        const user = await prisma.user.findUnique({
            where: { email },
            select: {
        ...
        const scored = candidats.map((film) => {
            ...
            // Bonus note IMDB
            if (film.imdbNote && film.imdbNote >= 7)
                score += 2;
            return { ...film, score };
        });
    ...
    fastify.get("/profil/:email/calendar", async (req, reply) => {
        const { email } = req.params;
        const user = await prisma.user.findUnique({
            where: { email },
            select: {
        ...
        const uid = (id) => `${id}@cineradar.fr`;
        const toIcsDate = (d) => d.toISOString().replace(/[-:.]/g, "").slice(0, 8);
