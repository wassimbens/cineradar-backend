...
function parseVersion(raw) {
    ...
    if (upper.includes("VOST"))
        return client_1.Version.VOSTFR;
    if (upper.startsWith("VO"))
        return client_1.Version.VOSTFR; // En France VO = Version Originale Sous-Titrée
...
function parseFormat(raw) {
    ...
    if (upper.includes("IMAX"))
        return "IMAX";
    if (upper.includes("DOLBY"))
        return "Dolby Atmos";
    if (upper.includes("3D"))
        return "3D";
    return "2D";
}
/**
 ...
 */
function parseFilmLinks(html, cinemaId) {
    const $ = cheerio.load(html);
    const films = [];
    const seen = new Set();
 ...
 * }
 */
function parseFilmMetadata(html) {
    const $ = cheerio.load(html);
    const meta = {};
    // 1. JSON-LD
...
function parseSeancesFromPost(html, cinemaId, dayStr) {
    ...
    cinemaBlock.find(".screening-time-start").each((_, el) => {
        ...
        let version = client_1.Version.VF; // valeur par défaut
        let $node = $(el).parent();
        let depth = 0;
        ...
        if (contextText.includes("IMAX"))
            format = "IMAX";
        else if (contextText.includes("DOLBY"))
            format = "Dolby Atmos";
        else if (/\b3D\b/.test(contextText))
            format = "3D";
        // ── Salle ────────────────────────────────────────────────────────
...
// ── Scraper principal ─────────────────────────────────────
class UgcScraper extends base_scraper_js_1.BaseScraper {
    name = "ugc";
    browser = null;
    context = null;
    ...
    async newPage() {
        if (!this.context)
            throw new Error("Contexte navigateur non initialisé");
    ...
    async fetchHtml(url) {
        ...
        try {
            ...
            if (!response)
                throw new Error(`Pas de réponse pour ${url}`);
            ...
            if (response.status() === 429) {
                ...
                this.log(`🚦 429 Too Many Requests — pause ${waitMs / 1_000}s avant retry…`, "warn");
     ...
     */
    async fetchCinemaNumericId(pageUrl, nom) {
        const page = await this.newPage();
        try {
            ...
            try {
                const jsId = await page.evaluate(() => {
                    const w = globalThis; // eslint-disable-line @typescript-eslint/no-explicit-any
                    const raw = w.cinemaId ??
                        w.CINEMA_ID ??
                        w.__cinemaId__ ??
                        w.App?.cinemaId ??
                        w.config?.cinemaId ??
                    ...
                    const el = doc?.querySelector("[data-cinema-id]") ??
    ...
    async fetchCinemaIds() {
        ...
        for (const { slug, nom, id: hardcodedId } of PARIS_CINEMA_SLUGS) {
            try {
                ...
                const numericId = await this.withRetry(() => this.fetchCinemaNumericId(pageUrl, nom), `ID cinéma ${nom}`);
                if (numericId) {
    ...
    async fetchFilmsForCinema(cinemaId) {
        const url = `${BASE_URL}/showingsCinemaAjaxAction!getShowingsForCinemaPage.action` +
            `?cinemaId=${cinemaId}`;
        const html = await this.withRetry(() => this.fetchHtml(url), `films cinéma ${cinemaId}`);
        return parseFilmLinks(html, cinemaId);
    }
    // ── Étape 3 : métadonnées film ────────────────────────
    async fetchFilmMetadata(href) {
        ...
        const html = await this.withRetry(() => this.fetchHtml(url), `métadonnées film ${cleanHref}`);
        ...
        if (!meta.titre) {
            ...
            this.log(`  ⚠️ Titre introuvable — ${url} (${html.length} octets)\n     HTML: ${preview}`, "warn");
        }
        return meta;
    }
     ...
     */
    async fetchSeances(filmId, cinemaId, filmHref) {
        ...
        try {
            ...
            await page.route("**/*getShowingsByFilm*", async (route) => {
                ...
                if (dayMatch?.[1])
                    autoPostHtmls.set(dayMatch[1], html);
            ...
            try {
                ...
                days = [
                    ...new Set([...daysHtml.matchAll(/\b(20\d{2}-\d{2}-\d{2})\b/g)].map((m) => m[1])),
                ]
                    .sort()
                    .filter((d) => {
                    const dt = new Date(d);
            ...
            for (const [day, html] of autoPostHtmls.entries()) {
                ...
                if (dayDate < today || dayDate > horizon)
                    continue;
     ...
     */
    async fetchCinemaDetails(cinemaId, nomFallback, slug) {
        const url = slug
            ? `${BASE_URL}/cinema-${slug}.html`
            : `${BASE_URL}/cinema.html?id=${cinemaId}`;
        try {
            const html = await this.withRetry(() => this.fetchHtml(url), `détails cinéma ${cinemaId}`);
            const $ = cheerio.load(html);
            let adresse = "";
            let ville = "Paris";
            ...
            if ((!ville || ville === "Paris") && codePostal && codePostal !== "75000") {
                ...
                if (CP_VILLES[prefix])
                    ville = CP_VILLES[prefix];
            }
            return { adresse, ville, codePostal, latitude, longitude };
        }
        catch (err) {
            this.log(`Impossible de récupérer les détails du cinéma ${cinemaId} : ${err}`, "warn");
            return {
                adresse: "",
                ville: "Paris",
                codePostal: "75000",
        ...
        try {
            ...
            for (const { id: cinemaId, nom: cinemaName, slug } of cinemaList) {
                ...
                try {
                    ...
                    this.addError(result, `Cinéma ${cinemaName} — impossible de lister les films : ${err}`);
                ...
                for (const { filmId, href } of filmLinks) {
                    ...
                    try {
                        ...
                        this.addError(result, `Film ${filmId} @ ${cinemaName} — métadonnées introuvables : ${err}`);
                    }
                    try {
                        seances = await this.fetchSeances(filmId, cinemaId, href);
                    }
                    catch (err) {
                        this.addError(result, `Film ${filmId} @ ${cinemaName} — séances introuvables : ${err}`);
                    }
                    if (!filmMeta.titre) {
                    ...
                    const cinemaFilm = { film, seances };
                    cinema.films.push(cinemaFilm);
                    this.log(`  ✓ "${film.titre}" — ${seances.length} séance(s)`);
                }
                result.cinemas.push(cinema);
...
exports.UgcScraper = UgcScraper;
//# sourceMappingURL=ugc.scraper.js.map