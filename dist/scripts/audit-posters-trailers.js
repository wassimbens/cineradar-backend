...
function score(dbFilm, tmdb) {
    const titleScore = Math.max(similarity(norm(dbFilm.titre), norm(tmdb.title)), similarity(norm(dbFilm.titre), norm(tmdb.original_title)), dbFilm.titreOriginal ? similarity(norm(dbFilm.titreOriginal), norm(tmdb.original_title)) : 0);
...
//# sourceMappingURL=audit-posters-trailers.js.map