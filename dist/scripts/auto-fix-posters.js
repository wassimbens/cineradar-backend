...
// ── TMDB ─────────────────────────────────────────────────
async function tmdbSearch(titre, annee, lang = "fr-FR") {
    if (!TMDB_API_KEY)
        return null;
    const trySearch = async (query, year) => {
...
async function tmdbResolve(film) {
    ...
    if (s1)
        return tmdbById(s1.id);
    if (s3)
        return tmdbById(s3.id);
    return null;
}
// ── YouTube thumbnail via TMDB ────────────────────────────
async function tmdbGetYoutubeThumbnail(tmdbId) {
    if (!TMDB_API_KEY)
        return null;
    ...
    try {
        ...
        if (await verifyImageUrl(maxres))
            return maxres;
...
// ── OMDB ──────────────────────────────────────────────────
async function omdbFetchPoster(titre, annee) {
    if (!OMDB_API_KEY)
        return null;
    const params = new URLSearchParams({
        apikey: OMDB_API_KEY,
...
async function allocineFetchPoster(titre, annee) {
    try {
        ...
        $(".entity-card, .meta-body-item, .thumbnail-container").each((_, el) => {
            ...
            const cardYear = parseInt(card.find("[class*='year'], .meta-body-info").text().match(/\d{4}/)?.[0] ?? "0", 10);
            let score = 0;
            if (annee && cardYear === annee)
                score += 2;
            else if (annee && Math.abs(cardYear - annee) <= 1)
                score += 1;
            else if (!annee)
                score += 1;
            // Extraire l'URL du poster
            const imgSrc = card.find("img[src*='acsta.net']").first().attr("src") ??
...
async function filterBrokenPosters(films) {
    const broken = [];
    for (const film of films) {
        ...
        if (film.affiche.includes("image.tmdb.org"))
            continue;
...
async function main() {
    ...
    for (const film of films) {
        ...
        try {
            ...
            const updateData = {};
            if (detail)
                updateData["tmdbId"] = String(detail.id);
            if (posterValid && posterUrl)
                updateData["affiche"] = posterUrl;
            ...
            if (detail) {
                ...
                if (!film.realisateur && detail.credits?.crew) {
                    ...
                    if (dir)
                        updateData["realisateur"] = dir.name;
                }
            ...
            try {
                ...
                const isP2002 = typeof updateErr === "object" &&
                    updateErr !== null &&
                    "code" in updateErr &&
                    updateErr.code === "P2002";
                if (isP2002 && updateData["tmdbId"]) {
                    const { tmdbId: _dropped, ...rest } = updateData;
...
//# sourceMappingURL=auto-fix-posters.js.map