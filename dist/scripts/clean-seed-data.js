...
async function main() {
    ...
    const { count: seancesDeleted } = await prisma_js_1.prisma.seance.deleteMany({
        where: { source: "seed" },
    });
    ...
    for (const tmdbId of SEED_TMDB_IDS) {
        const film = await prisma_js_1.prisma.film.findUnique({ where: { tmdbId } });
        if (!film)
            continue;
        const seancesCount = await prisma_js_1.prisma.seance.count({ where: { filmId: film.id } });
        if (seancesCount === 0) {
            // Nettoyer d'abord les alertes liées
            await prisma_js_1.prisma.alerte.updateMany({
                where: { filmId: film.id },
                data: { filmId: null },
            });
            await prisma_js_1.prisma.film.delete({ where: { id: film.id } });
    ...
    for (const nom of SEED_CINEMA_NOMS) {
        const cinema = await prisma_js_1.prisma.cinema.findFirst({
            where: { nom: { equals: nom, mode: "insensitive" } },
            include: { salles: { include: { seances: true } } },
        });
        if (!cinema)
            continue;
        const totalSeances = cinema.salles.reduce((acc, s) => acc + s.seances.filter((seq) => seq.source !== "seed").length, 0);
        if (totalSeances === 0) {
            // Cascade supprime salles + séances
            await prisma_js_1.prisma.cinema.delete({ where: { id: cinema.id } });
...
//# sourceMappingURL=clean-seed-data.js.map