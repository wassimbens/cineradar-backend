...
async function fetchCollection(collectionId) {
    try {
        ...
        const res = await fetch(`${TMDB_BASE}/collection/${collectionId}?${params}`, { signal: AbortSignal.timeout(10_000) });
        if (!res.ok)
            return null;
        return res.json();
    }
...
async function main() {
    ...
    const collectionsFound = new Map();
    ...
    for (let i = 0; i < films.length; i++) {
        ...
        const missing = col.parts.filter(p => !existingTmdbIds.has(String(p.id)) &&
            p.poster_path &&
            p.release_date &&
        ...
        for (const part of missing) {
            ...
            try {
                ...
                if (err.code === "P2002") {
                    ...
                    console.error(`  ❌ "${titre}" : ${err.message?.slice(0, 80)}`);
                    totalErrors++;
                }
            }
        }
...
//# sourceMappingURL=complete-franchises.js.map