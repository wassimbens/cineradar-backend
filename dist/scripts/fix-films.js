...
async function main() {
    ...
    marios.forEach((f) => console.log(`  id=${f.id} titre="${f.titre}" tmdb=${f.tmdbId} seancesTotal=${f._count.seances}`));
    ...
    for (const t of targets) {
        ...
        if (film)
            console.log(`[${film._count.seances} séances/30j] "${film.titre}" (${film.annee}) tmdb=${film.tmdbId}`);
    }
    await prisma.$disconnect();
}
main().catch((err) => { console.error(err); process.exit(1); });
//# sourceMappingURL=fix-films.js.map