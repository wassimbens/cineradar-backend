...
async function checkUrl(label: string, url: string): Promise<{ status: number; body: string }> {
...
async function main() {
  ...
  const ugcList = await checkUrl(
    "UGC cinémas liste region=1",
    "https://www.ugc.fr/cinemasAjaxAction!getCinemasList.action?region=1"
  );
  ...
  const allocineCinema = await checkUrl(
    "AlloCiné cinéma C0159",
  ...
  if (allocineCinema.status === 200) {
    ...
    $("[class]").each((_, el) => {
      ...
      cls.split(/\s+/).forEach(c => { if (c && c.length > 3) allClasses.add(c); });
    });
  ...
  await checkUrl(
    "AlloCiné région Paris",
    "https://www.allocine.fr/salle/region_gen_r_5767.html"
  );
