...
function pickCanonical(group: FilmRow[]): FilmRow {
  // Score : tmdbId présent + nb séances + nb avis + nb favoris
  return [...group].sort((a, b) => {
    const aHasTmdb = a.tmdbId ? 1 : 0;
    const bHasTmdb = b.tmdbId ? 1 : 0;
    if (aHasTmdb !== bHasTmdb) return bHasTmdb - aHasTmdb;
    if (a._count.seances !== b._count.seances) return b._count.seances - a._count.seances;
    return 0;
  })[0];
}

async function mergeFilms(canonicalId: string, dupId: string): Promise<{
  seances: number; avis: number; filmsVus: number;
  favoris: number; watchlist: number;
}> {
  ...
  for (const f of dupFav) {
    ...
    if (existing) await prisma.filmFavori.delete({ where: { id: f.id } });
    else await prisma.filmFavori.update({ where: { id: f.id }, data: { filmId: canonicalId } });
    counts.favoris++;
  }
  ...
  for (const w of dupW) {
    ...
    if (existing) await prisma.watchlistItem.delete({ where: { id: w.id } });
    else await prisma.watchlistItem.update({ where: { id: w.id }, data: { filmId: canonicalId } });
    counts.watchlist++;
  }
...
async function main() {
  ...
  for (let i = 0; i < films.length; i++) {
    for (let j = i + 1; j < films.length; j++) {
      const a = films[i];
      const b = films[j];
      if (!a.annee || !b.annee || Math.abs(a.annee - b.annee) > 1) continue;
  ...
  const dupePairs: Array<{ canonical: FilmRow; dup: FilmRow }> = [];
  ...
  for (const group of groups.values()) {
    ...
    if (uniqGroup.length < 2) continue;
