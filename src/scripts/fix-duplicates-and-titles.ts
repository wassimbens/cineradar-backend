...
async function mergeFilms(keepId: string, deleteId: string) {
...
async function main() {
    ...
    for (const film of allFilms) {
      ...
      for (const variant of group) {
        ...
        if (normTitre === normVariant || normOrig === normVariant) {
          if (!matches.find(m => m.id === film.id)) matches.push(film);
        }
      }
    }
  ...
  if (suspectTitles.length > 0) {
    ...
    for (const f of suspectTitles) console.log(`    id=${f.id} "${f.titre}" (${f.annee})`);
  }
