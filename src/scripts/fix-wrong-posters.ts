...
async function searchTmdb(query: string, year?: number): Promise<TmdbMovie | null> {
  try {
...
async function findFilmInDb(titre: string): Promise<{ id: string; titre: string; tmdbId: string | null; affiche: string | null } | null> {
  // Recherche exacte
...
async function main() {
  ...
  for (const target of FILMS_TO_FIX) {
    ...
    if (!tmdbMovie && target.titreOriginal) {
      ...
      if (!tmdbMovie && target.annee) tmdbMovie = await searchTmdb(target.titreOriginal);
    }

    // 3. Recherche par titre français
    if (!tmdbMovie) {
      tmdbMovie = await searchTmdb(target.titre, target.annee);
      if (!tmdbMovie && target.annee) tmdbMovie = await searchTmdb(target.titre);
    }
