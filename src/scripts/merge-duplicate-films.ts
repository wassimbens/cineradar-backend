...
async function main() {
  ...
  for (const group of duplicateGroups) {
    ...
    for (const dup of duplicates) {
      console.log(
        `    └─ "${dup.titre}" (${dup.id.slice(-6)})  ${dup._count.seances} séances`
      );
    }
    ...
    for (const dup of duplicates) {
      ...
      const existingKeys = new Set(
        existingSeances.map(s => `${s.salleId}|${s.dateHeure.getTime()}`)
      );
      ...
      if (!canFull.tmdbId && dupFull.tmdbId) enrichUpdate.tmdbId = dupFull.tmdbId;
      if (!isTmdbUrl(canFull.affiche) && isTmdbUrl(dupFull.affiche))
        enrichUpdate.affiche = dupFull.affiche;
      if (!canFull.synopsis && dupFull.synopsis) enrichUpdate.synopsis = dupFull.synopsis;
      if (!canFull.duree && dupFull.duree) enrichUpdate.duree = dupFull.duree;
