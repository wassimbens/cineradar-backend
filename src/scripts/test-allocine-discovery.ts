...
function extractIdsFromText(text: string, ids: Set<string>): void {
  for (const m of text.matchAll(/salle_gen_csalle=([A-Z][A-Z0-9]*)/g)) ids.add(m[1]);
  for (const m of text.matchAll(/"(?:internalId|theaterId|theaterCode)"\s*:\s*"([A-Z][A-Z0-9]*)"/g)) ids.add(m[1]);
  for (const m of text.matchAll(/theater-([A-Z][A-Z0-9]*)\b/g)) ids.add(m[1]);
}
...
async function main() {
  ...
  if (ids.size > 0) {
    ...
    if (ids.size > 30) console.log(`   … et ${ids.size - 30} autres`);
  }
}
